...
public class PlanServiceImpl implements PlanService {
    ...
    public TravelPlanResponse createPlan(TravelPlanRequest travelPlanRequest) {
        List<TravelPlanRequest.PlaceDTO> selectedPlaces = travelPlanRequest.getSelectedPlaces();
        ...
        for (TravelPlanRequest.PlaceDTO place : selectedPlaces) {
        ...
        prompt.append("응답은 반드시 JSON 형식으로만 해.\n\n");
        prompt.append("{\n");
        ...
        String aiResponse = openAIService.generateTravelPlan(prompt.toString());

        // aiResponse를 TravelPlanResponse로 변환
        try {
            ...
            List<TravelPlanResponse.ScheduleDto> schedules = new ArrayList<>();
            DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm:ss");
            for (Map<String, Object> scheduleEntry : schedulesMap) {
                TravelPlanResponse.ScheduleDto schedule = TravelPlanResponse.ScheduleDto.builder()
                        .dateTime(LocalDateTime.parse((String) scheduleEntry.get("dateTime"), formatter))
                        .placeName((String) scheduleEntry.get("name"))
                        .address((String) scheduleEntry.get("address"))
                        .transport((String) scheduleEntry.get("transport"))
                        .latitude((String) scheduleEntry.get("latitude"))
                        .longitude((String) scheduleEntry.get("longitude"))
                        .build();
            ...
            return TravelPlanResponse.builder()
                    ...
                    .transport(travelPlanRequest.getTransport())
                    .schedules(schedules)
                    .build();
        } catch (Exception e) {
            throw new RuntimeException("Failed to parse the response: " + e.getMessage(), e);
        }
    }
}
